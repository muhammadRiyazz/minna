...
class _ScreenConfirmTicketState extends State<ScreenConfirmTicket> {
  ...
  void _initializeFares() {
    ...
    updatedFare =
        double.tryParse(widget.blockResponse.fareBreakup?.updatedFare ?? '0') ??
        totalFare;
    updatedServiceTax =
        double.tryParse(
          widget.blockResponse.fareBreakup?.updatedServiceTax ?? '0',
        ) ??
        0.0;
  ...
  Widget _buildFareComparisonRow(
    String label,
    double amount, {
  ...
  Future<Map<String, dynamic>> _preparePaymentOptions(String orderId) async {
    ...
    Map<String, dynamic> options = {
      ...
      'name': 'MT Trip',
      'description':
          'Bus Ticket Booking - ${widget.selectedSeats.length} seat(s)',
      'order_id': orderId,
  ...
  void _handlePaymentSuccess(PaymentSuccessResponse response) async {
    log(
      "Payment Success - Payment ID: ${response.paymentId}, Order ID: ${response.orderId}",
    );
    ...
    try {
      ...
      if (!saveResult['success']) {
        throw Exception(
          "Failed to save payment details: ${saveResult['message']}",
        );
      }
    ...
    if (Theme.of(context).platform == TargetPlatform.iOS) {
      if (errorCode != null) {
        ...
        if (response.message?.toLowerCase().contains('cancelled') == true) {
          errorMessage = "Payment cancelled by user";
        } else if (response.message?.toLowerCase().contains('network') ==
            true) {
          errorMessage = "Network error. Please check your connection";
        }
      }
  ...
  Future<void> _handleBookingFailure(String paymentId, String error) async {
    try {
      ...
      if (mounted) {
        _showErrorDialog(
          "Booking failed. ${refundResult['success'] ? 'Refund has been initiated successfully.' : 'Failed to initiate refund. Please contact support.'}",
          errorDetails: error,
        );
      }
    } catch (refundError) {
  ...
  Widget _buildPassengerSection() {
    return Column(
      ...
      children: [
        ...
        SizedBox(height: 16),
        ...widget.alldata.inventoryItems?.map(
              (item) => Container(
