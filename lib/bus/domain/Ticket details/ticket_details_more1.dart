...
class TicketinfoMore {
  ...
  factory TicketinfoMore.fromJson(Map<String, dynamic> json) {
    final inventoryItemJson = json['inventoryItems'];

    List<InventoryItem> inventoryItems = [];
    if (inventoryItemJson is List) {
      inventoryItems = List<InventoryItem>.from(
        inventoryItemJson.map((x) => InventoryItem.fromJson(x)),
      );
    } else if (inventoryItemJson is Map) {
      inventoryItems = [
        InventoryItem.fromJson(inventoryItemJson as Map<String, dynamic>)
      ];
    }
