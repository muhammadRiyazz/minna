...
class ApiStatus {
  ...
  ApiStatus({required this.code, required this.description});
...
class PreBookRoom {
  ...
  Map<String, dynamic> toJson() {
    ...
    data['DayRates'] = dayRates
        .map((roomRates) => roomRates.map((rate) => rate.toJson()).toList())
        .toList();
    ...
    data['Supplements'] = supplements
        .map(
          (roomSupplements) =>
              roomSupplements.map((supplement) => supplement.toJson()).toList(),
        )
        .toList();
...
class DayRate {
  ...
  factory DayRate.fromJson(Map<String, dynamic> json) {
    return DayRate(basePrice: (json['BasePrice'] ?? 0).toDouble());
  }
...
class Member {
  ...
  factory Member.fromJson(Map<String, dynamic> json) {
    return Member(
      ...
      isPrimaryAgent: json['isPrimaryAgent'] ?? false,
    );
  }
...
class ApiError {
  ...
  ApiError({required this.errorCode, required this.errorMessage});
...
class PreBookRequest {
  ...
  PreBookRequest({required this.bookingCode, this.paymentMode = 'Limit'});
...
class PrebookCallbackResponse {
  ...
  bool get isSuccess =>
      status.toUpperCase() == 'SUCCESS' || status.toLowerCase() == 'true';
...
class PrebookCallbackData {
  ...
  PrebookCallbackData({required this.prebookId});

  factory PrebookCallbackData.fromJson(Map<String, dynamic> json) {
    return PrebookCallbackData(prebookId: json['prebookId'] ?? 0);
  }

  Map<String, dynamic> toJson() {
    return {'prebookId': prebookId};
  }
}

// mttrip.in PreBook API Response Wrapper
class PreBookApiResponse {
  final bool status;
  final String message;
  final PreBookResponse? data;

  PreBookApiResponse({required this.status, required this.message, this.data});

  factory PreBookApiResponse.fromJson(Map<String, dynamic> json) {
    return PreBookApiResponse(
      status: json['status'] ?? false,
      message: json['message'] ?? '',
      data: json['data'] != null
          ? PreBookResponse.fromJson(json['data'])
          : null,
    );
  }
